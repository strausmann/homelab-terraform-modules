...
variable "template_tags" {
  type        = list(string)
  default     = ["latest", "os-ubuntu", "v-2404", "docker"]
  description = "Tags zum automatischen Finden des Docker-VM-Templates"
}
...
variable "vm_description" {
  type        = string
  default     = "Docker Node — Managed by OpenTofu"
}

variable "vm_tags" {
  type        = list(string)
  default     = ["opentofu", "linux", "docker"]
}
...
variable "disk_size" {
  type        = number
  default     = 50
  description = "OS-Disk-Groesse in GB"
}

variable "storage_pool" {
  type        = string
  default     = "local-lvm"
  description = "Proxmox Storage Pool fuer die Disks"
}
...
variable "docker_disk_size" {
  type        = number
  default     = 300
  description = "Groesse der Docker-Daten-Disk in GB (wird unter /docker gemountet)"
}

variable "docker_disk_interface" {
  type        = string
  default     = "scsi1"
  description = "Disk-Interface fuer die Docker-Daten-Disk"
}
